...
interface StepPurchaseOrderFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  editingOrder?: any;
...
export function StepPurchaseOrderForm({
  isOpen,
  onClose,
  onSuccess,
  editingOrder,
}: StepPurchaseOrderFormProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (editingOrder) {
        setValue("poNumber", editingOrder.poNumber);
        setValue("supplierName", editingOrder.supplierName);
        setValue("supplierEmail", editingOrder.supplierEmail);
        setValue("status", editingOrder.status);
        setValue("orderDate", editingOrder.orderDate);
        setValue("expectedDelivery", editingOrder.expectedDelivery);
      } else {
        reset();
        setCart([]);
        generatePoNumber();
  ...
  const fetchFrequentItems = async () => {
    ...
    try {
      // Filter products by supplier if supplier name is provided
      let filteredItems = products;
      if (supplierName && supplierName.length > 2) {
        filteredItems = products.filter((product) =>
          product.supplier.toLowerCase().includes(supplierName.toLowerCase())
        );
      }
      setFrequentItems(filteredItems.slice(0, 5));
    } catch (error) {
  ...
  const updatePurchasePrice = (productId: string, price: number) => {
    setCart(
      cart.map((item) =>
        item.id === productId ? { ...item, purchase_price: price } : item
      )
    );
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
        ...
        {/* Content */}
        <div className="p-6" style={{ height: "calc(90vh - 200px)" }}>
          {error && (
